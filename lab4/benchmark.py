...
class DynamicArrayBenchmark(Benchmark):
    ...
    def benchmark_remove(self) -> BenchmarkCallback:
        n: int = 2000
        random_values: List[int] = sample(range(n), n)
...
class SortingBenchmark(Benchmark):
    ...
    def benchmark_counting_sort(self) -> BenchmarkCallback:
        def callback() -> None:
            counting_sort_through_public_api(self.linked_list, key=lambda x: x)

        return callback, 1

    def benchmark_gnome_sort(self) -> BenchmarkCallback:
        def callback() -> None:
